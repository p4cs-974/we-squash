...
## Proper QR Code generator for Godot 4
## Generates a valid QR code texture from text data
...
## Generate a QR code texture from text
## Returns an ImageTexture with the QR code
static func generate_qr_texture(text: String, module_px_size: int = 8) -> ImageTexture:
	var generator := QRCodeGenerator.new()
	return generator.get_texture(text, module_px_size)

func get_texture(input: String, module_px_size: int = 8) -> ImageTexture:
	var data: Array = get_data(input)
	return _generate_texture_image(data, module_px_size)
...
func _set_encoding_type(value: String) -> void:
	...
	for character in value:
		is_alphanumeric = is_alphanumeric && QRUtils.ALPHANUMERIC_CHARACTERS.has(character)
...
func _encode_numeric(value: String) -> void:
	...
	for index in data.size():
		...
		if data[index] < 10:
			size = 4
		qr_data_list.append_array(QRUtils.convert_to_binary(data[index], size))

func _encode_alphanumeric(value: String) -> void:
	qr_data_list = []
	var data: Array = []
	for index in range(0, ceil(value.length() / 2.0)):
		var first_value: int = QRUtils.ALPHANUMERIC_CHARACTERS[value[index * 2]]
		var second_value: int = -1
		if index * 2 + 1 < value.length():
			second_value = QRUtils.ALPHANUMERIC_CHARACTERS[value[index * 2 + 1]]
		
		var result: Array
		if second_value != -1:
			result = QRUtils.convert_to_binary(first_value * 45 + second_value, 11)
		else:
			result = QRUtils.convert_to_binary(first_value, 6)
...
func _encode_bytes(value: String) -> void:
	...
	var byte_array = value.to_utf8_buffer()
	for byte in byte_array:
		qr_data_list.append_array(QRUtils.convert_to_binary(byte))
...
func _set_info_segments() -> void:
	...
	var length_bits = QRUtils.convert_to_binary(qr_data_length, length_bits_size)
...
func _set_error_correction() -> void:
	...
	for block_index in range(qr_data_list.size()):
		...
		for index in block_bytes.size():
			block_bytes[index] = QRUtils.convert_to_decimal(block_bytes[index])
		
		block_bytes = rs.get_remainder(block_bytes)
		
		for byte in block_bytes:
			ecc_data_list[block_index].append_array(QRUtils.convert_to_binary(byte, 8))
...
func _generate_texture_image(data: Array, module_px_size: int) -> ImageTexture:
	# Add quiet zone (4 modules border)
	var quiet_zone: int = 4
	var size: int = data.size() + (quiet_zone * 2)
	var image: Image = Image.create(size * module_px_size, size * module_px_size, false, Image.FORMAT_RGBA8)
	...
	for row in range(data.size()):
		for col in range(data[row].size()):
			...
			# Draw module at scaled position with quiet zone offset
			var px_row = (row + quiet_zone) * module_px_size
			var px_col = (col + quiet_zone) * module_px_size
			
			for dy in range(module_px_size):
				for dx in range(module_px_size):
					image.set_pixel(px_col + dx, px_row + dy, color)
